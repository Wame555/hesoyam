...
bool BinanceUserStream::create_listen_key(){
    ...
    try{ auto j = json::parse(r.text); listen_key_ = j.value("listenKey", std::string("")); }
    catch(...){ spdlog::error("listenKey parse error: {}", r.text); }
    return !listen_key_.empty();
}

bool BinanceUserStream::start(){
    if (api_key_.empty()) { spdlog::warn("No API key for user stream"); return false; }
...
void BinanceUserStream::keepalive_loop(){
    // 30-minute keepalive (Binance docs allow 60m; we stay conservative)
    while (running_.load()){
        ...
        if (r.status_code>=300) spdlog::warn("userStream keepalive HTTP {}", r.status_code);
    }
}
...
void BinanceUserStream::connect_ws(){
    ...
    ws_.setOnMessage([this](const ix::WebSocketMessagePtr& msg){
        ...
        switch (msg->type){
            case WebSocketMessageType::Open: spdlog::info("User WS open"); break;
            case WebSocketMessageType::Message: handle_msg(msg->str); break;
            case WebSocketMessageType::Close:
            case WebSocketMessageType::Error: spdlog::warn("User WS closed/error: {} {}", msg->closeInfo.code, msg->errorInfo.reason);
                if (running_.load()) { create_listen_key(); connect_ws(); }
                break;
            default: break;
        }
    });
...
void BinanceUserStream::handle_msg(const std::string& s){
    try{
        ...
        if (ev=="executionReport"){
            ExecUpdate u; u.orderId = j.value("i", 0ULL); u.symbol = j.value("s", std::string(""));
            u.side = j.value("S", std::string("")); u.status = j.value("X", std::string(""));
            u.lastQty = to_d(j, "l"); u.lastPrice = to_d(j, "L"); u.cumQty = to_d(j, "z");
